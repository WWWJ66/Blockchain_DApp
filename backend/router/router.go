...
func SetupRouter() *gin.Engine {
	...
	// 获取水产品的上链信息
	r.POST("/getFishInfo", con.GetFishInfo)
	// 获取用户的水产品ID列表
	r.POST("/getFishList", middleware.JWTAuthMiddleware(), con.GetFishList)
	// 获取所有的水产品信息
	r.POST("/getAllFishInfo", middleware.JWTAuthMiddleware(), con.GetAllFishInfo)
	// 获取水产品上链历史(溯源)
	r.POST("/getFishHistory", middleware.JWTAuthMiddleware(), con.GetFishHistory)
	return r
}
