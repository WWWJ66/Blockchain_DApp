        ...
        <div class="container" style="width: 33.33%; margin: 0 auto; text-align: center;">
            <form @submit.prevent="handleSubmit" style="text-align: left; max-width: 300px; margin: 0 auto;">
                ...
                <div style="text-align: center; margin-top: 20px;">
                    <button @click="goToRegistration" style="width: auto; max-width: none; background-color: transparent; border: none; color: rgb(43, 43, 57); text-decoration: underline; font-size: 70%; cursor: pointer; font-family: 'Microsoft YaHei', sans-serif; font-weight: bold;">没有账户？立即注册</button>
                </div>
            </form>
        </div>
    </div>
    ...
    methods: {
        handleSubmit() {
            fetch('your-backend-url', {
                ...
                body: JSON.stringify({ phone: this.phone, password: this.password, role: this.role })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    alert('登录成功');
                    this.$router.push({ name: 'InformationEntry' });
                } else if (data.error === 'password') {
                    alert('密码错误，请重新输入');
                } else if (data.error === 'role') {
                    alert('角色错误，请重新选择');
                } else {
                    alert('未知错误');
                }
